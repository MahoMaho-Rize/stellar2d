...
|---|---|---|
| `docs/derivations/mhd/sections/*.md` | **Formal derivation book.**  Ships in `manuscript.pdf`. | **English only.** |
| `docs/projects/mhd_verification/*.md` | Benchmark logs, milestone records, debugging memos. | Chinese / English mixed OK. |
...
2026-05-08 session.  Rebuilding `manuscript.pdf` after B-M5.5
surfaced: a missing `}` in §F5 silently killed the build while the
script reported success; four long `\boxed{}` blocks and a few long
blockquotes produced >100 pt Overfull hbox.  Then translating the
Chinese implementation memos I had added to §B4 / §C6 showed that
the derivation book had no CJK support and the memos didn't belong
there anyway — they belong in `docs/projects/mhd_verification/`.

Result: this guide plus the normaliser, enforced by
`build_manuscript.sh`.  Authors write plausible markdown; the build
