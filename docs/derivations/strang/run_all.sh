#!/usr/bin/env bash
# Run every sympy derivation script in docs/derivations/strang/scripts/.
# Stops on first failure (any assert_zero violation) and reports the
# offending script.  Also emits Part-D *.goldens.json used by
# tests/test_strang_*.cu — so this script is a prerequisite of ctest.
#
# Usage:
