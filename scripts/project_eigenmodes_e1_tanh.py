...
def main():
    ...
    # (4) |c_c(t)| linear early-time fit.
    ax = axes[1, 1]
    c_amp = np.sqrt(np.clip(E_c, 1e-50, None))
    noise = c_amp[0] if c_amp[0] > 1e-30 else c_amp[1:5].mean()
    # Early growth regime: pick t in (0, T_beat/4) periods, signal > noise
    t_early_max = min(T_beat / T_a / 4, n_per[-1] * 0.3)
    mask_e = (n_per > 0.5) & (n_per < t_early_max) & (c_amp > 3 * noise)
    ax.loglog(n_per[1:] + 1e-2, c_amp[1:], "o", color="#2ca02c", ms=3,
              label="$|c_c(t)|$")
    if mask_e.sum() >= 3:
        t_fit = n_per[mask_e]
        c_fit = c_amp[mask_e]
        coef_fit = np.polyfit(np.log(t_fit), np.log(c_fit), 1)
        tref = np.linspace(t_fit.min(), t_fit.max(), 30)
        ax.loglog(tref, np.exp(coef_fit[1]) * tref ** coef_fit[0],
                  "k--", lw=1.3,
                  label=f"early-time fit: $\\propto t^{{{coef_fit[0]:.2f}}}$")
        # ∝ t reference.
        ax.loglog(tref, c_fit[0] * tref / t_fit[0], "r:", lw=1.0,
                  label="$\\propto t$ (linear-triad theory)")
    ax.set_xlabel("periods of mode $a$  (log)")
    ax.set_ylabel("$|c_c(t)|$")
    ax.set_title("Mode-$c$ amplitude growth")
    ax.legend(loc="best", fontsize=8)
