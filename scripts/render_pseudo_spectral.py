#!/usr/bin/env python3
"""Fast 1080p renderer for pseudo_spectral KH VTK -> MP4.

Forked from render_cart_ale.py. 偽譜法只有兩個有意義的場:
  - ω  (渦度)  存在 VTK 的 'e_int' scalar 欄位,色帶對稱於 0 (RdBu_r)
  - |v| (速度大小) 存在 'mach' 欄位 (不可壓無音速,複用該欄位),plasma 色帶
無密度、熵、壓力 (都是 solver 填的佔位值)。

Usage: python render_pseudo_spectral.py <run_dir> [output.mp4] [fps] [workers]
"""
...
def render_one(args):
    ...
    if fields is None or "e_int" not in fields:
        return None

    omega = fields["e_int"]                        # 渦度存在 e_int 欄位
    speed = fields.get("mach", np.zeros((_ny, _nx)))  # |v| 存在 mach 欄位
...
def main():
    ...
    # Pre-scan color ranges (抽樣 20 幀)
    sample_idx = np.linspace(0, len(files) - 1, min(20, len(files))).astype(int)
    w_all, sp_all = [], []
    for si in sample_idx:
        f = parse_vtk_cells(files[si], nx, ny, {"e_int", "mach"})
        if f is None:
            continue
        w_all.append(f["e_int"].ravel())
