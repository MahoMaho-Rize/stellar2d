...
# render_kh.parse_vtk_dims 回傳 (nx, ny, Lx, Ly);偽譜版只回 (nx, ny),
# 這裡加適配。
from render_kh import parse_vtk_cells, parse_vtk_dims as _parse_vtk_dims_full


def parse_vtk_dims(path):
    nx, ny, _, _ = _parse_vtk_dims_full(path)
    return nx, ny


def parse_vtk_extent(path):
    return _parse_vtk_dims_full(path)
...
def main():
    ...
    nx, ny, Lx, Ly = parse_vtk_extent(files[0])
    print(f"Grid: {nx}×{ny}  domain {Lx:.3f}×{Ly:.3f}  processing {len(files)} frames")
    ...
    for i, path in enumerate(files):
        ...
        k, Ek, KE = compute_spectrum(u, v, Lx=Lx, Ly=Ly)
        m = (k > 0) & (Ek > 1e-30)
    ...
    # Nyquist cutoff (ALE 沒有 2/3 dealias,但 grid-scale k_Ny = π/dx)
    dx_grid = Lx / nx
    k_nyq = np.pi / dx_grid
    ax.axvline(k_nyq, color="red", alpha=0.3, lw=1, ls="--")
    ax.text(k_nyq, ax.get_ylim()[1] * 0.3,
            " Nyquist", color="red", alpha=0.6, fontsize=9,
