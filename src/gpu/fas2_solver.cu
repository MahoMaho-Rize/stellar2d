...
static void alloc_fas_level(FasLevel2& lev) {
    ...
    // GMRES scratch
    for (int i = 0; i <= FasLevel2::GMRES_K; ++i)
...
void FasSolver2::build_level(int l, int nr, int nt, int ng,
...
void FasSolver2::init(const Grid& grid, const EOS& eos_in, double G, double cfl) {
    eos = eos_in;
    gamma = eos.gamma;
    G_const = G;
    cfl_num = cfl;
...
void FasSolver2::upload_state(const Grid& grid, const State& state) {
...
void FasSolver2::download_state(const Grid& grid, State& state) {
    FasLevel2& lev = levels[0];
    int nr = lev.nr, nt = lev.nt, ng = lev.ng;
    int stride = nt + 2*ng;
    int total = (nr+2*ng)*stride;
...
void FasSolver2::snapshot_hse() {
    ...
    for (int l = 1; l < n_levels; ++l) {
        FasLevel2& fl = levels[l-1], &cl = levels[l];
        int cn = cl.nr * cl.nt;
...
__global__
void k_fas2_bdf2_rhs(const double* Un, const double* Un_prev,
...
void FasSolver2::jfnk_matvec(const double* d_v, double* d_Jv, double dt, double g0_over_dt) {
    FasLevel2& lev = levels[0];
...
void FasSolver2::apply_preconditioner(const double* d_v, double* d_Mv,
...
int FasSolver2::gmres_solve(double* d_x, const double* d_b, double dt,
                            double g0_over_dt, double tol, int max_iter) {
    FasLevel2& lev = levels[0];
    int n = lev.nr * lev.nt, N = 4*n, B = 256;
    int m = std::min(max_iter, (int)FasLevel2::GMRES_K);
...
int FasSolver2::solve(double dt, double g0_over_dt, int max_cycles, double tol) {
    FasLevel2& lev = levels[0];
    int n = lev.nr * lev.nt, N4 = 4*n, B = 256;
...
double FasSolver2::step(double t, double t_end) {
    ...
    if (interior_volume > 0) {
        ...
        if (fabs(dM) > 1e-30 || fabs(dE) > 1e-30) {
            k_fas2_conserve_correct<<<(n+B-1)/B,B>>>(
    ...
    if (radial_only) {
        ...
        k_fas2_zero_mt<<<(total+B-1)/B,B>>>(finest.d_mt, finest.nr, finest.nt, finest.ng);
    }
...
__global__
void k_fas2_rk_update(double* rho, double* mr, double* mt, double* rhoE,
...
__global__
void k_fas2_rk_average(double* rho, double* mr, double* mt, double* rhoE,
...
double FasSolver2::step_explicit(double t, double t_end) {
    if (!hse_set) { snapshot_hse(); }
    ...
    if (radial_only) {
        int total = lev.total;
        k_fas2_zero_mt<<<(total+B-1)/B,B>>>(lev.d_mt, lev.nr, lev.nt, lev.ng);
    }
...
void FasSolver2::destroy() {
    for (int l = 0; l < n_levels; ++l) {
        ...
        for (int i = 0; i <= FasLevel2::GMRES_K; ++i) cudaFree(lev.d_gmres_V[i]);
        for (int i = 0; i < FasLevel2::GMRES_K; ++i) cudaFree(lev.d_gmres_Z[i]);
