...
void FasSolver2::restrict_state(int fine, int coarse) {
    FasLevel2& fl = levels[fine], &cl = levels[coarse];
    int cn = cl.nr * cl.nt, B = 256;
    k_fas2_restrict_state<<<(cn+B-1)/B,B>>>(
...
__global__
void k_fas2_assemble_coarse_rhs(
...
void FasSolver2::restrict_defect(int fine, int coarse, double g0_over_dt) {
    FasLevel2& fl = levels[fine], &cl = levels[coarse];
    int fn = fl.nr * fl.nt, cn = cl.nr * cl.nt, B = 256;

    compute_F(fine, g0_over_dt);

    k_fas2_restrict_defect<<<(cn+B-1)/B,B>>>(
    ...
    k_fas2_assemble_coarse_rhs<<<(cn+B-1)/B,B>>>(
        cl.d_res,
        cl.d_rho, cl.d_mr, cl.d_mt, cl.d_rhoE,
        cl.d_Un,
        cl.d_fas_rhs,
...
__global__
void k_fas2_prolongate_correct(
...
void FasSolver2::prolongate_correct(int coarse, int fine) {
    FasLevel2& cl = levels[coarse], &fl = levels[fine];
    int cn = cl.nr * cl.nt, B = 256;

    // cl.d_save contains the pre-solve restricted state
    k_fas2_prolongate_correct<<<(cn+B-1)/B,B>>>(
...
void FasSolver2::fas_vcycle(int l, double dt, double g0_over_dt) {
