"""
Ideal-radiation EOS focused Lane-Emden diagnostic plot.

Compared to plot_lane_emden.py, this version emphasizes angular symmetry
breaking, including north-south (dipole-like) asymmetry along the z-axis.
"""
import argparse
import glob
import os
...
def main():
    parser = argparse.ArgumentParser(description='Plot ideal+radiation EOS symmetry diagnostics.')
    parser.add_argument('--input-dir', default=None, help='Directory containing output_*.vtk and run.log')
    args = parser.parse_args()

    repo_root = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..')
    build_dir = args.input_dir or find_default_input_dir(repo_root)

    vtk_init = os.path.join(build_dir, 'output_0000.vtk')
    vtk_files = sorted(glob.glob(os.path.join(build_dir, 'output_*.vtk')))
    if not vtk_files:
        raise RuntimeError(f'No output_*.vtk files found in {build_dir}')
    vtk_last = max(vtk_files, key=os.path.getmtime)

    nr, nt, r_init, data_init, coords_init = read_vtk(vtk_init)
    nr_final, nt_final, r_final, data_final, coords_final = read_vtk(vtk_last)
    if nr_final != nr or nt_final != nt:
        raise RuntimeError(
            f"Mismatched outputs: init is {nr}x{nt}, latest file '{os.path.basename(vtk_last)}' is "
            f"{nr_final}x{nt_final}. Clean old VTK files or rerun in a fresh directory."
        )
    ...
    density_init = data_init['density']
    j_eq = nt // 2
    r_1d = r_init[:, j_eq]
    rho_init_1d = density_init[:, j_eq]
    rho_final_1d = density_final[:, j_eq]
    ...
    north_south = np.abs(density_final[:, 0] - density_final[:, -1]) / np.maximum(pole_mean, 1e-30)
    ...
    fig.suptitle('Ideal+Radiation EOS Symmetry Diagnostics (Lane-Emden IC)', fontsize=14)

    ax = axes[0, 0]
    ax.plot(r_analytic, rho_analytic, 'k-', lw=2, label='analytic')
    ax.plot(r_1d, rho_init_1d, 'b--', lw=1.5, label='t = 0')
    ax.plot(r_1d, rho_final_1d, 'r.', ms=5, label='t = final')
    ax.set_xlabel('r')
    ax.set_ylabel(r'$\rho$')
    ax.set_title('(a) Equatorial radial density profile')
    ax.legend()
    ax.set_xlim(0, R_star * 1.1)
    ...
    ax.set_title('(b) Equatorial relative density error')
    ax.legend()
    ax.set_xlim(0, R_star * 1.1)
    ...
    spike_theta = 0.5 * (
        np.arctan2(x_n[i_max, j_max], z_n[i_max, j_max]) +
        np.arctan2(x_n[i_max + 1, j_max + 1], z_n[i_max + 1, j_max + 1])
    )
    ax.plot(
    ...
    ax.set_xlabel('x (= r sin θ)')
    ax.set_ylabel('z (= r cos θ)')
    ax.set_title(f'(c) 2D density map (true max = {rho_max:.2e})')
    ax.set_aspect('equal')
    ...
    ax.semilogy(r_1d, north_south, color='tab:orange', lw=1.6, ls='-.', label='north/south mismatch')
    ax.set_xlabel('r')
    ax.set_ylabel('asymmetry metric')
    ax.set_title('(d) Angular asymmetry diagnostics')
    ax.axhline(1.0, color='k', ls=':', lw=0.5)
    ax.legend(fontsize=8)

    log_path = os.path.join(build_dir, 'run.log')
    times, masses = parse_mass_log(log_path)
    if len(masses) >= 2:
        M0 = masses[0]
        dM = np.max(np.abs((masses - M0) / M0))
        ax.text(
            0.03, 0.03,
            f'max |ΔM/M0| = {dM:.2e}',
    ...
    tend = times[-1] if len(times) else 0.0
    run_cmd = f'./stellar2d --test lane_emden --eos ideal_rad --nr {nr} --ntheta {nt} --tend {tend:.3f}'
    add_provenance_footer(fig, __file__, run_cmd=run_cmd)

    plt.tight_layout(rect=[0, 0.03, 1, 1])
    out_dir = os.path.join(repo_root, 'results')
    os.makedirs(out_dir, exist_ok=True)
    out_name = build_filename('lane_emden', 'ideal_rad_diagnostics', nr, nt, tend, ext='png')
    out_path = os.path.join(out_dir, out_name)
    fig.savefig(out_path, dpi=150, bbox_inches='tight')
    print(f'Saved: {out_path}')
    print(f'Input dir: {build_dir}')
    print(f'Latest file: {os.path.basename(vtk_last)}')
    print(f'Max density: {rho_max:.6e} at (i={i_max}, j={j_max}, r={r_spike:.6e})')
    print(f'Max pole/equator ratio: {np.max(pole_to_eq):.6e}')
    print(f'Max theta std/mean: {np.max(theta_cv):.6e}')
    print(f'Max north/south mismatch: {np.max(north_south):.6e}')
